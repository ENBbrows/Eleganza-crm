...
This is a separate, self-contained app from the rest of this repo (the
Eleganza CRM) — different brand, different Supabase tables, different
users. Nothing here touches Eleganza's data.
...
`supabase/migrations/0016_blacktree_budgeting_tool.sql` → Run.
...
- **Site URL**: wherever you host this folder, e.g. `https://yourdomain.com/blacktree/`
- **Redirect URLs**: add `https://yourdomain.com/blacktree/dashboard.html`
...
These are plain static files — host the `blacktree/` folder anywhere that
serves static HTML over HTTPS (HTTPS is required for the PWA install
prompts and service worker to work at all). Point clients and agents at
`https://yourdomain.com/blacktree/` and send them to `setup-guide.html`
first.
